...
# vLLM worker processes must use spawn (not fork) when using CUDA, otherwise
# "Cannot re-initialize CUDA in forked subprocess". Set before importing vLLM.
# https://docs.vllm.ai/en/stable/usage/troubleshooting.html#python-multiprocessing
...
def save_checkpoint(results: list, output_file: str, mode: str = 'append'):
    ...
    # Normalize rows to avoid mixed schemas (e.g., dict vs string "{}")
    normalized = []
    for row in results:
        clean = {}
    ...
    if mode == 'append' and Path(output_file).exists():
        if output_file.endswith('.parquet'):
            ...
            # Checkpoints can legitimately gain a value in a column that was
            # entirely null in an earlier batch. Relaxed concatenation keeps
            # those nullable columns compatible across long-running runs.
            combined = pl.concat([existing, df], how="diagonal_relaxed", rechunk=True)
...
def process_with_vllm(
    ...
    processor = VLLMNLPProcessor(
        ...
        tokenizer=os.environ.get("VLLM_TOKENIZER"),
    )
...
def main():
    ...
    parser.add_argument('--quantization', '-q', default=None,
                       help='vLLM quantization (e.g. awq). Auto-detected from model id when omitted.')
